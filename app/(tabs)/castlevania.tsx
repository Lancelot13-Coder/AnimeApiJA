...
export default function CastlevaniaScreen() {
  const { setUltimoCastlevania } = useContext(AnimeContext);
  ...
  const buscar = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/castlevania/nombre/${encodeURIComponent(nombre.trim().toLowerCase())}`);
  ...
  const cargarLista = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/castlevania`);
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      ...
      <Text style={styles.title}>🧛 CASTLEVANIA</Text>
      <Text style={styles.subtitle}>Busca un personaje</Text>

      <TextInput
        style={styles.input}
        placeholder="ej: Alucard, Trevor, Sypha..."
      ...
      <Modal animationType="slide" transparent visible={modalLista} onRequestClose={() => setModalLista(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>📋 Personajes — Castlevania</Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => { setPersonaje(item); setUltimoCastlevania(item); setModalLista(false); setNombre(item.nombre); }}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 30, color: '#f59e0b', fontWeight: 'bold', textAlign: 'center', letterSpacing: 3, marginBottom: 4 },
