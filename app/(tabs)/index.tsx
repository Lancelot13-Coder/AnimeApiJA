...
export default function ChainsawManScreen() {
  const { setUltimoChainsawMan } = useContext(AnimeContext);
  ...
  const buscar = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/chainsawman/nombre/${encodeURIComponent(nombre.trim().toLowerCase())}`);
  ...
  const cargarLista = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/chainsawman`);
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      ...
      <Text style={styles.title}>🪚 CHAINSAW MAN</Text>
      <Text style={styles.subtitle}>Busca un personaje</Text>

      {/* Input nombre */}
      <TextInput
        style={styles.input}
        placeholder="ej: Denji, Makima, Power..."
      ...
      <Modal animationType="slide" transparent visible={modalLista} onRequestClose={() => setModalLista(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>📋 Personajes — Chainsaw Man</Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => { setPersonaje(item); setUltimoChainsawMan(item); setModalLista(false); setNombre(item.nombre); }}
                >
...
const styles = StyleSheet.create({
  ...
  title:       { fontSize: 30, color: '#e74c3c', fontWeight: 'bold', textAlign: 'center', letterSpacing: 3, marginBottom: 4 },
